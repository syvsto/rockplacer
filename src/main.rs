...
fn main() {
    let mut source_image = image::open("slate.jpg")
        .expect("failed to read image")
        .to_luma();

    let mut clusters = generate_clusters(&mut source_image, 2);
    kmeans_iteration(&mut source_image, &mut clusters);
    kmeans_iteration(&mut source_image, &mut clusters);
    kmeans_iteration(&mut source_image, &mut clusters);
    kmeans_finalize(&mut source_image, &mut clusters);
